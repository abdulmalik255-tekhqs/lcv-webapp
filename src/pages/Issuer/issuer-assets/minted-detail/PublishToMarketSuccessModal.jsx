...
const PublishToMarketSuccessModal = ({ isOpen, onClose, asset, transactionData }) => {
  ...
  const txData = {
    ...
    txHash: apiData?.transaction_hash || apiData?.txHash || "0x8f7e6d5c4b3a2f1e0d9c8b7a6f5e4d3c2b1a0f9e8d7c6b5a4f3e2d1c",
    blockNumber: apiData?.block_number || apiData?.blockNumber || "15,847,392",
    network: apiData?.network || "Canton Network",
    timestamp: apiData?.timestamp 
      ? new Date(apiData.timestamp).toLocaleString("en-US", {
          month: "2-digit",
          day: "2-digit",
          year: "numeric",
          hour: "2-digit",
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[13px] font-normal text-[#000]">
              Your asset has been successfully published to the marketplace and is now available for investment.
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="flex gap-3">
        <Button variant="gradient" onClick={() => navigate("/issuer/assets")} className="flex-1 h-[40px]">
          Close
        </Button>
      </div>
    </GenericModal>
...
export default PublishToMarketSuccessModal;
