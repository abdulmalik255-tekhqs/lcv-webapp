...
const AssetImage = ({ src, alt, name }) => {
  ...
  if (!src || imageError) {
    return (
      <div className="h-10 w-10 flex items-center justify-center bg-[#FAFAFC] text-[#48484A] text-xs font-medium">
...
const mapAssetData = (apiAsset) => {
  ...
  return {
    ...
    address: apiAsset.address || null, // Contract address if available
  };
};
...
function RegistrarAssetsSection() {
              ...
              <table className="w-full text-left text-[13px] border border-[#E5E5EA]">
                <thead className="bg-[#FAFAFC] border-b border-[#E5E5EA]">
                  <tr className="text-[13px] font-[700]">
                    ...
                    <th className="px-4 py-3 text-left">Latest Update</th>
                  ...
                  {visibleAssets.map((asset, index) => {
                    ...
                    const handleRowClick = () => {
                      switch (asset.statusType) {
                        case "pending":
                          navigate(
                            `/registrar/assets/pending-review/${asset.id}`
                          );
                          break;
                        case "approved":
                          navigate(
                            `/registrar/assets/approved-details/${asset.id}`
                          );
                          break;
                        case "minted":
                          navigate(
                            `/registrar/assets/minted-details/${asset.id}`
                          );
                          break;
                        // case "published":
                        //   navigate(
                        //     `/registrar/assets/published-details/${asset.id}`
                        //   );
                        //   break;
                        case "denied":
                          navigate(
                            `/registrar/assets/denied-details/${asset.id}`
                          );
                          break;
                        default:
                          break;
                      ...
                      >
                        <td className="px-3 py-5">
                          <div className="flex items-center gap-2">
                            <div className="flex-shrink-0 border border-[#E5E5EA] bg-[#FAFAFC] rounded-md p-2 flex items-center justify-center">
                              {asset.iconUrl ? (
                                <AssetImage
                                  src={asset.iconUrl}
                                  alt={asset.name}
                        ...
                        >
                          ...
                          {asset.statusType === "minted" && (
                            <button
                              ...
                              className="w-fit text-[#48484A] hover:text-[#000] cursor-pointer pl-7"
                            >
...
export default RegistrarAssetsSection;
