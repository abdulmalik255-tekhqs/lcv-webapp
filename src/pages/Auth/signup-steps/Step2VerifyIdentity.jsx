...
import { useSignupOtpVerify, useSignupOtpRequest } from "../../../api";

const CODE_LENGTH = 6;

const Step2VerifyIdentity = ({
  formData,
  onNext,
  onBack,
  onUpdateFormData,
  ...
  const otpRequestCode =
    formData.otpRequestCode || searchParams.get("otpRequestCode") || "";

  const [code, setCode] = useState(
    formData.otp ? formData.otp.split("") : Array(CODE_LENGTH).fill("")
  );
  const inputRefs = useRef([]);
  const otpVerifyMutation = useSignupOtpVerify();
  const otpRequestMutation = useSignupOtpRequest();

  const isLoading = otpVerifyMutation.isPending || otpRequestMutation.isPending;
  ...
  useEffect(() => {
    ...
    // Defer focus to avoid issues during render
    const timer = setTimeout(() => {
      inputRefs.current[0]?.focus();
    }, 0);
    return () => clearTimeout(timer);
  }, [email, otpRequestCode, navigate]);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (response?.code) {
        ...
        onNext({ otp, otpVerifyCode: response.code });
      }
    } catch (error) {
      const errorMessage =
  ...
  const handleResend = async () => {
    ...
    const invitationId =
      formData.invitationId || searchParams.get("invitation_id") || "";

    try {
      const otpPayload = { email: email };
      if (invitationId) {
        otpPayload.invitation_id = invitationId;
      }

      const response = await otpRequestMutation.mutateAsync(otpPayload);

      if (response?.code) {
        // Defer state updates to avoid updating during render
        setTimeout(() => {
          onUpdateFormData({ otpRequestCode: response.code });
          showBottomRightToast(`We sent a fresh verification code to ${email}.`);
          setCode(Array(CODE_LENGTH).fill(""));
          // Focus after state updates complete
          setTimeout(() => {
            inputRefs.current[0]?.focus();
          }, 0);
        }, 0);
...
export default Step2VerifyIdentity;
