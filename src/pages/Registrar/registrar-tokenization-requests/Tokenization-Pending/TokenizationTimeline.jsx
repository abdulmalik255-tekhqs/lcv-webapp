...
const TokenizationTimeline = ({ currentStep = 2 }) => {
  const [completedSteps, setCompletedSteps] = useState([1]); // Step 1 is already completed
  ...
  return (
    <div className="space-y-4">
      <span className="text-[15px] font-medium text-[#000] !font-['Montserrat']">Tokenization Timeline</span>
      <div className="relative">
        {/* Vertical line */}
        <div className="absolute left-2.5 top-0 bottom-0 w-0.5 bg-gray-200"></div>

        {steps.map((step, index) => {
          const isStepCompleted = completedSteps.includes(step.id) || step.completed;
            ...
            >
              ...
              <div className="flex-1 pt-1">
                <div className="flex items-center gap-2">
                  <p className={`text-sm font-medium ${isStepCompleted || isCurrent ? "text-[#000]" : "text-[#000]"}`}>
                    {step.label}
                  </p>
                  {hasActionRequired && (
                    <span className="text-xs font-medium text-[#0734A9]">Action Required</span>
                  )}
                </div>
                {step.date && (
                  <p className={`text-xs ${isStepCompleted || isCurrent ? "text-gray-600" : "text-[#000]"}`}>
                    {step.date}
                  </p>
                )}
              </div>
...
export default TokenizationTimeline;
